...
def guardar_calendario(fecha, calendario_dict):
    """
    Guarda la planificación completa de una semana (todos los días) en la BD.
    Usa INSERT OR REPLACE para no duplicar.
    """
    conn = sqlite3.connect('calendario.db')
    cursor = conn.cursor()
...
def cargar_semana(fecha):
    """
    Carga la semana solicitada. Si no existe, hereda de la semana anterior más cercana.
    Siempre devuelve un dict con los 7 días (lista vacía si no hay datos).
    """
    ...
    try:
        ...
        if not df.empty:
            return _df_to_dict(df, dias_semana)

        # 2. Buscar la semana anterior más reciente
        cursor = conn.cursor()
        cursor.execute(
        ...
        if row and row[0]:
            ...
            if not df_prev.empty:
                return _df_to_dict(df_prev, dias_semana)

        # 3. Sin historial: devolver semana vacía
        return {dia: [] for dia in dias_semana}
...
# Obtener datos para mostrar en la tabla principal (con herencia)
